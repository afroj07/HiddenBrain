...
export const createPodcast = async(podcast, token)=>{
  await API.post('/podcasts',podcast,  {headers:{"Authorization":`Bearer ${token}`}},
  {withCredentials:true});
  }
  ...
  export const addEpisodes = async(podcast, token)=>{
    await API.post('/podcasts/episode/', podcast ,{headers:{"Authorization":`Bearer ${token}`}},
    {withCredentials:true});
    }
    
    export const favoritePodcast = async(id, token)=>{
      await API.post('/podcasts/favorit/', {id:id} ,{headers:{"Authorization":`Bearer ${token}`}},
      {withCredentials:true});
      }
