...
import { googleSingIn, signUp } from "../api/index";
import OTP from "./OTP";
...
const SignUp = ({ setSignUpOpen, setSignInOpen }) => {
      ...
      try {
        signUp({ name, email, password }).then((res) => {
          if (res.status === 200) {
            ...
            dispatch(
              openSnackbar({
                message: "Otp verified & Account created successfuly",
                severiy: "success",
              })
            );
            setLoading(false);
  ...
  const validatePassword = () => {
    if (password.length < 8) {
      setcredentialError("Password must be atleast 8 charcters long!");
      setPasswordCorrect(false);
    } else if (password.length > 16) {
      setcredentialError("Passord must be less than 16 characters long!");
      setPasswordCorrect(false);
    } else if (
  ...
  const validateName = () => {
    if (name.length < 4) {
      setcredentialError("Name must be atleast 4 charcters long!");
